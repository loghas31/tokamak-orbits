...
### 24. `RETRACTED` — "more coils means more ripple wells"

Writing the ripple-well test in `test_ripple.py`, I asserted that at fixed edge
ripple the well fraction grows with coil number:
...
The reasoning was that $N$ sits in the denominator of
$\alpha_{\rm rt} = \epsilon|\sin\theta|/(Nq\delta)$, so more coils means smaller
$\alpha$ means more well. That is half the story. At **fixed edge ripple** the
amplitude profile is $\delta(R) = \delta_{\rm edge}(R/R_{\rm edge})^N$, which
falls faster with $N$ everywhere inside the boundary — at $r = 0.25$ m,
$\delta$ drops from $7.3\times10^{-3}$ at $N = 8$ to $2.9\times10^{-3}$ at
$N = 32$. The product $N\delta$ therefore saturates, and the well fraction peaks:

| $N$ | 8 | 12 | 16 | 20 | 24 | 32 |
|---|---|---|---|---|---|---|
| well fraction, $r = 0.25$ m | 0.108 | 0.125 | **0.131** | 0.131 | 0.128 | 0.118 |

The test failed on the first run, which is the system working. What is worth
recording is *why* I wrote it: I reasoned from one term of an expression I had
just written down myself, and asserted the conclusion without evaluating it. The
replacement test asserts the shape that is actually there — an interior maximum,
with both tails below it — and says in its docstring that the first version was
wrong, so the same reasoning cannot be repeated by someone reading it later.

Note that this is a statement about *fixed edge ripple*, which is the comparison
the code makes. It is not the engineering statement "more coils reduce ripple",
which is about $\delta_{\rm edge}$ itself and is true.

---

### 25. `BUG` — a collision timestep that was small at the start and meaningless at the end

The energy operator in `collisions_full.py` was applied on a fixed stride, with
`collide_every` chosen so that $\nu\Delta t_{\rm coll} \approx 10^{-3}$ at
launch. That is the same policy §4b of `NUMERICS.md` prescribes for the Lorentz
operator, and for that operator it is correct, because there $\nu_d$ is a
constant.

It is wrong here. The collision frequencies go as $v^{-3}$, and the §9.3
experiment follows a 100 keV ion through a **full slowing-down** to a 10 keV
background. By the end of the run the low-speed tail of the population was being
stepped at $\nu\Delta t_{\rm coll} = 2.0$.

What that costs, measured on a relaxation from a cold beam that should settle at
$\langle E\rangle/T_b = 1.5$:

| raw $\nu\Delta t$ | no sub-cycling | sub-cycled to 0.005 |
|---|---|---|
| 0.02 | 1.526 | 1.499 |
| 0.10 | 1.644 | 1.493 |
...
Six orders of magnitude. Past $\nu\Delta t \sim 0.5$ the diffusive increment
$\sqrt{B\Delta t}$ exceeds the speed itself and the population runs away.

**The reason this survived a review is the part worth keeping.** Every other
diagnostic in that run was perfect — energy error $10^{-13}$, divergence
$10^{-9}$, orbit topology correct, the operator's own unit tests all green —
because none of them touch the collision operator at low speed. The unit tests
started from a Maxwellian and stayed there, so they never visited the regime
where the step is invalid. A test suite that only exercises the intended
operating point cannot find an error that only appears when the run leaves it.

The published number from that version was a final mean energy of 14.17 keV
against a 0-D prediction of 15.49 keV, quoted with a ratio of 0.915. It was
retracted before it reached `RESULTS.md`.

Fixed by sub-cycling per particle: each collision step is split into
$\lceil\nu_\parallel\Delta t_{\rm coll}/(\nu\Delta t)_{\max}\rceil$ sub-steps.
The operator now reports `max_nu_dt_seen`, `max_substeps_used` and
`substep_cap_hits`, the last of which counts particle-steps that wanted more
sub-cycling than the cap allowed — so a run that *did* integrate part of itself
too coarsely says so instead of looking clean.

This is finding 22's lesson for the third time (after the detrapping exponent and
the OOM of finding 23): a step size validated against one quantity is not
validated against another. The difference is that this time the requirement is
enforced by the code rather than written down in a document.

---

### 26. `RETRACTED` + `WEAKER` — an adversarial review of the three new modules, and what it took back

The Solov'ev equilibrium, the energy-scattering operator and the field ripple
were written, tested (77 new tests, all passing), documented, and written up in
`RESULTS.md` §8–§10. A hostile review pass was then run against those files
alone, with instructions to find things that are wrong rather than to confirm.

It found thirty. Nine of them touched a claim in the prose. They are grouped
below by what kind of mistake they are, because the pattern is more useful than
the list.

#### (a) Statistics that are zero, or one, by construction

**"Ripple loss is one-directional" — retracted.** §10.3 reported that no
particle the axisymmetric field loses is confined once ripple is added
("rescued = 0 in every row") and read that as evidence the perturbation only
loses particles rather than reshuffling them. But `rescued` was measured only
against the $\delta = 0$ control, whose five losses are prompt counter-going
orbits gone in ~12 µs — nothing can rescue those, so the statistic **could not
have been anything but zero**. Comparing *adjacent* amplitudes instead: three
particles lost at $\delta_{\rm edge} = 0.5\%$ are confined at both 1% and 2%.
Ripple does move particles both ways. The section now reports the
adjacent-amplitude moves, and the one-directionality claim is gone.

**"Cold and hot starts reach the same Maxwellian" — was one run, not two.**
§9.2's strongest-sounding sentence was that two very different initial
conditions relax to the same distribution "to three digits". Both were run with
`seed=303`. Under common random numbers the dynamics contracts and the two
trajectories synchronise: 99.3% of the particles were **bitwise identical**
between the cold and hot runs, and the KS statistics agreed to all 17 digits —
which is what should have given it away. Fixed by giving the two starts
independent seeds and re-running.

**The Monte-Carlo drag check cannot fail.** §9.1 presented
$\langle\Delta v\rangle/\Delta t$ against the analytic drift as a validation.
But `scatter_speed` computes `v + mu*dt + sign*sqrt(B*dt)` with a symmetric
sign, so the sample mean *is* `mu` plus zero-mean noise. The ratio is 1 by
construction and the only defect it can detect is a biased sign draw. It is a
closure check on the sampler, and is now labelled as one in both the code and
the prose.

#### (b) A validation that never touched the thing it validated

**The Grad–Shafranov residual is floating-point cancellation.** §8.1 called it
"a finite-difference measurement … not an algebraic identity, so it is a real
check". It is an algebraic identity: $\psi$ is quartic in $R$ and quadratic in
$Z$, both central differences are *exact*, and the residual is analytically zero
for any $(c, k, R_0)$. What was quoted to three figures is roundoff, and it
scales as $1/h^2$ — $7\times10^{-12}$ at $h = 10^{-3}$, $8.9\times10^{-6}$ at
$h = 10^{-6}$. Changing one unquoted parameter moves the "result" by six orders
of magnitude.

Worse: the residual never evaluates `b_field`. Mutating $F^2(\psi)$ so that the
toroidal field no longer integrates the $FF'$ the source term claims — a genuine
equilibrium-breaking change — leaves **all 28 equilibrium tests passing**.

Fixed by adding `force_balance_residual`, which builds
$\mathbf{J} = \nabla\times\mathbf{B}/\mu_0$ by differencing `b_field` itself and
tests $\mathbf{J}\times\mathbf{B} = \nabla p$ against the same $p'$. On the
correct field it returns $2.7\times10^{-10}$; on the mutated one, $1.4\times10^{-1}$.
Nine orders of magnitude, and the Grad–Shafranov residual sees none of it. There
is now a test asserting exactly that contrast, and another asserting that the
old residual *rises* as $h$ shrinks, so nobody quotes it as physics again.

#### (c) A dimensional error hidden by a unit-valued parameter

`gwb_threshold` returned a quantity with units of inverse length. Its
denominator was $\rho\,\delta\,(\partial\ln\delta/\partial r)\,R_0$, which is
metres, against a dimensionless numerator. This was invisible because the
default machine has $R_0 = 1$ m; scaling the machine at fixed
$\epsilon, \delta, N, q, \rho$ gives $\sigma \propto R_0^{-3/2}$. It had also
substituted $\partial\ln\delta/\partial r$ for the $q'$ the real
Goldston–White–Boozer criterion uses, and matched neither of the two forms
written in its own docstring. Rewritten with the actual $dq/dr$, and there is now
a test that scales the machine and asserts $\sigma$ does not move.

#### (d) Prose that drifted from the data

- §10.1 quoted the naive ripple divergence as $1.6\times10^{-2}$; the JSON says
  $2.65\times10^{-2}$.
- §10.1 said the ripple "varies by a factor of 67 across the minor radius". 66.5
  is axis-to-outboard-boundary; across the *full* minor radius it is
  $(1.3/0.7)^{16} \approx 2\times10^{4}$.
- §10.3 claimed the two loss channels "stay separate" in time. Two of the three
  $\delta = 1\%$ ripple losses are at 14.6 and 17.5 µs, inside the prompt window
  (11.1–14.6 µs).
- §10.3 said the prompt loss median is "unchanged by ripple to within a
  microsecond". It runs 12.42 → 12.04 → 11.78 → 11.54 → 11.13 µs: a spread of
  1.29 µs and **monotonic in $\delta$**, i.e. a systematic trend reported as no
  trend.
- §9.3 attributed $\nu\Delta t = 39.2$ to the bulk slowing down "to a fifth of
  its speed". The bulk ends at 0.40 of its initial speed and
  $\nu\Delta t = 5.0\times10^{-4}$ — four orders of magnitude away. The large
  value comes from sub-eV particles in the low-speed tail, where the
  frequencies go as $v^{-2}$. Finding 25 had this right and §9.3 did not.
  Compounding it, the figure quoted was $\nu_\perp$ — the *pitch* frequency —
  in a run where pitch scattering was switched off; with the counter fixed to
  report the operator that actually ran, the number is $\nu_\parallel\Delta t
  = 19.6$, and the speed floor turns out never to be reached (0 hits).
- §9.1's "$\nu_s$ alone is wrong by a factor of two" is the $x\to\infty$ limit,
  not a general statement; below $v = 0.80\,v_{\rm th}$ the speed drag changes
  sign entirely.
- The module docstring for `equilibrium.py` described the Shafranov shift
  backwards relative to §8.2, which had it right.
- `SolovevField.q_axis` was documented as "approximate" for elongated cases. The
  realised $q_0$ is **exactly** $\kappa$ times the requested value — 80% off at
  $\kappa = 1.8$. Now asserted by a test rather than excused by a word.
- `ripple.py` called $R^{+N}$ "the only $Z$-independent harmonic solution".
  $R^{-N}$ is equally harmonic; $R^{+N}$ is the one *regular on the axis*.
- `ripple_well_fraction` averaged over poloidal *angle*, not area. The wells sit
  at $\theta \approx 0$, exactly where the surface element $R_0 + r\cos\theta$
  is largest, so the reported fraction understated the area fraction by 24%
  (0.131 against 0.163).
- `RippledField` on a `SolovevField` base raised `AttributeError` in both
  ripple criteria — `SolovevField` has `safety_factor_exact`, not
  `safety_factor` — while §10.1 claimed the wrapper "applies to the Solov'ev
  equilibrium without modification, and the tests assert that it does". Only the
  divergence test covered that combination.

#### (e) The library default contradicted the document that justified it

`MaxwellianCollisions(max_nu_dt=0.02)` cited §9.2 as authority for the value
0.02. §9.2's own table marks $\nu\Delta t = 0.02$ as **failing** its KS test and
concludes that $\leq 0.005$ is the requirement. The default is now 0.005.

Relatedly, `substep_cap_hits` — the counter that finding 25 introduced
specifically so that under-resolved steps could not hide — was computed from the
speed *after* the step, not the speed the sub-step decision was made from, so it
was not counting the steps that were actually capped. It is now returned by
`scatter_speed` itself, together with counters for the speed floor and the pitch
clip, which were the two remaining places the operator silently modified a value
in a module whose stated ethos is that it does not.

#### (f) Tests that assert the implementation back to itself

The review named about a dozen: `test_source_terms_are_constant` restates two
return statements; `test_boundary_label_is_a` asserts `return self.a`;
`test_delta_edge_is_what_it_says` asserts the normalisation that defines
`delta_0`; `test_midplane_is_always_in_a_well` holds for every possible
parameter set because $\sin 0 = 0$; `test_axisymmetric` is unconditional because
`b_magnitude` only ever sees $(R, z)$.

These are **not** deleted. Cheap identity tests are worth having as change
detectors — they catch a refactor that quietly alters a definition. What was
wrong was counting them towards "77 tests pass" as if all 77 carried equal
weight. The count is a floor on effort, not a measure of coverage, and the
mutation test in (b) is the honest measure: it found a class of error that 28
passing tests did not.

---

**What this finding is really about.** Findings 1–25 were each a single mistake.
This one is a mistake about *method*: every item above was written after the
code passed its tests, and most of them were written **by me, in the same
session, while trying to be careful**. Three of the nine retracted claims were
sentences whose whole rhetorical purpose was to sound rigorous — "not an
algebraic identity, so it is a real check", "the same numbers to three digits",
"zero in every row". The confident phrasing was the tell.

The practical lesson: a statistic that comes out exactly 0, exactly 1, or
identical across two runs should be treated as **broken until proven otherwise**,
not as a strong result.
