...
Read `docs/DOC_STATUS.md` before quoting any of this. The short version: this is
a divergence-free toy field with tokamak-like topology, not an MHD equilibrium,
and there are no collisions. Several agreements below are weaker than they look,
and where that is true it is said here rather than left for the reader to find.
...
## 8. Things that did not work, or are not settled
...
- **Collisions are pitch-angle only.** No energy scattering, no drag, no
  slowing-down, and no momentum conservation between species — so there is still
  no bootstrap current, and nothing here is a complete neoclassical calculation.
  What §7 adds is the trapped-particle detrapping physics, not transport
  coefficients.
