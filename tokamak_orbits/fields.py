...
class TokamakField(Field):
    ...
    # -- generic flux-surface interface ------------------------------------
    # Shared with SolovevField so diagnostics work against either field
    # without knowing which. surface_label is a monotonic radial coordinate in
    # metres, zero on the magnetic axis and equal to `a` at the boundary.
    def surface_label(self, pos):
        """Minor radius of the flux surface through ``pos`` (m)."""
        return self.flux_coords(pos)[0]
    ...
    def b_max_in_domain(self):
        """Largest ``|B|`` anywhere inside the plasma boundary (T).

        Sets the production timestep: resolving the gyro-period at the
        *strongest* field a particle can reach makes "40 steps per gyro-period"
        a guarantee rather than an average. Exposed as a property so the same
        policy applies to any field, including one whose surfaces are not
        circles.
        """
        return float(self.b_toroidal_at(self.R0 - self.a)
                     * np.sqrt(1.0 + (self.b_poloidal_reference(self.a) / self.B0) ** 2))

    def b_extrema_on_surface(self, pos):
        """``(B_min, B_max)`` on the flux surface through each position.

        Analytic here: both field components carry the same ``R0/R`` factor,
        so ``|B| = (R0/R) sqrt(B0^2 + B_theta,ref(r)^2)`` and the extrema sit
        at the midplane, ``R = R0 -/+ r``. That exact ``1/R`` behaviour is
        precisely what makes the trapped-fraction comparison an identity for
        this field -- see ``docs/DOC_SELF_REVIEW.md`` finding 4, and
        :mod:`tokamak_orbits.equilibrium` for a field where it is not true.
        """
        r = self.surface_label(pos)
        scale = np.sqrt(self.B0**2 + self.b_poloidal_reference(r) ** 2)
        return (self.R0 / (self.R0 + r)) * scale, (self.R0 / (self.R0 - r)) * scale
