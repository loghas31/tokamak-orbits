...
def classify(field, traj, mass, charge, smooth_gyroperiods: float = 1.0,
    ...
    r_gc, _ = field.flux_coords(gc.reshape(-1, 3))
...
def mirror_ratio(field, x):
    ...
    For an axisymmetric field with circular flux surfaces the maximum sits at
    the inboard midplane, ``R = R0 - r``.
    """
    x = np.atleast_2d(np.asarray(x, float))
    r, _ = field.flux_coords(x)
    R = np.hypot(x[..., 0], x[..., 1])
    b_local = np.linalg.norm(field.b_field(x), axis=-1)
    inboard = np.stack([field.R0 - r, np.zeros_like(r), np.zeros_like(r)],
                       axis=-1)
    b_max = np.linalg.norm(field.b_field(inboard), axis=-1)
...
def make_loss_func(field, r_limit=None):
    ...
    """
    r_limit = field.a if r_limit is None else r_limit

    def loss(pos):
        r, _ = field.flux_coords(pos)
        return r > r_limit
